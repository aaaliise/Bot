...
async def play(update, context):
    ...
    if update.message.text == "/play":
        ...
        await update.message.reply_text(
            ...
            f"Все города, которые можно называть, только из России.\n"
            f"1.Город, который тебе нужно назвать, начинается на последную букву предшесвующего города и удовлетворяет "
            f"правилу № 0, КРОМЕ ПЕРВОГО\n2.Первый город ты можешь назвать любой, удовлетворяющий правилу №0\n"
            f"3. Чтобы прекратить игру напиши (или выбери из меню) /stop\n"
            f"4. Все города нужно писать с большой буквы и соблюдать -, а также если город содержит два слова, "
            f"то оба писать с большой буквы и слитно.")
        return 8
    else:
        await update.message.reply_text("Ты завершил эту игру! Вызывай новую команду")
        return ConversationHandler.END
